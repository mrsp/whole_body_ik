...
nao_wbc::nao_wbc(ros::NodeHandle nh_)
{
    ...
    as_ = new actionlib::SimpleActionServer<whole_body_ik_msgs::HumanoidAction>(nh, "/nao_raisim_ros/whole_body_control", boost::bind(&nao_wbc::ActionServercontrolCb, this, _1), false);
    as_->start();
    ...
    jointNominalConfig << 0, 0, 0.32, 1, 0, 0, 0,
        ...
        1.5, -0.15, 0, 0.0349066, 1.5, 0;
    jointNominalVelocity.resize(32);
    jointNominalVelocity.setZero();
}

Vector3d nao_wbc::getDesiredLLegPosition()
{
    return desired_pin->linkPosition(lfoot_frame);
}
Vector3d nao_wbc::getDesiredRLegPosition()
{
    return desired_pin->linkPosition(rfoot_frame);
}
Quaterniond nao_wbc::getDesiredLLegOrientation()
{
    return desired_pin->linkOrientation(lfoot_frame);
}
Quaterniond nao_wbc::getDesiredRLegOrientation()
{
...
void nao_wbc::ActionServercontrolCb(const whole_body_ik_msgs::HumanoidGoalConstPtr msg)
{
    ...
    {
        if (msg->Joints[j].name.compare("LHand") != 0 && msg->Joints[j].name.compare("RHand") != 0)
        {
...
void nao_wbc::controlCb(Eigen::VectorXd &qd, Eigen::VectorXd& dqd, const whole_body_ik_msgs::HumanoidGoal msg)
{
